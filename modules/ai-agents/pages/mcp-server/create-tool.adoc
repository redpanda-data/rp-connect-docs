...
* xref:install:rpk.adoc[Redpanda Connect installed]
...
Each YAML file must contain exactly one component. The directory determines the component type:
...
xref:components:inputs/about.adoc[Inputs] read data from sources, xref:components:outputs/about.adoc[outputs] write data to destinations, and xref:components:caches/about.adoc[caches] store and retrieve data. Define these components directly at the top level:
...
. Connect to an AI client and verify the tool appears:
+
[source,bash]
----
