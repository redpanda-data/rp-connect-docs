...
This example shows a complete MCP tool that fetches current weather data from a public API. The tool demonstrates input validation, error handling, and structured responses.
...
<1> Input validation using xref:components:processors/mutation.adoc[`mutation`] to sanitize the city name and ensure it's not empty.
<2> A xref:components:processors/try.adoc[`try`] block to handle the API request and transform the response.
<3> xref:components:processors/http.adoc[`http`] processor fetches weather data from the wttr.in API with a 10-second timeout.
<4> Transform the API response into a structured JSON format with relevant weather fields.
<5> A xref:components:processors/catch.adoc[`catch`] block returns meaningful error messages when the API fails.
<6> The tool contract with `meta.mcp` defines how AI clients discover and invoke this tool.

When an AI assistant calls this tool with `{"city": "San Francisco"}`, it returns:
...
For more patterns and examples, see xref:ai-agents:mcp-server/pipeline-patterns.adoc[].
