...
. <<pipeline-patterns, Implement the pipeline>>: Build the logic for your MCP tool using Redpanda Connect pipelines.
...
[[contract]]
== Define the tool contract and MCP metadata
...
For example, to expose only tools tagged with `demo`, start the server with:
