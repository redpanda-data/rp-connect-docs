...
Redpanda Connect MCP server implements the open Model Context Protocol (MCP) for tool exposure. Only the tool concept from the MCP server specification is supported. Features such as MCP resources and prompts are not yet available.
