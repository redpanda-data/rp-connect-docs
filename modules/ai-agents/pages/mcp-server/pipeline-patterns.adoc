...
* **Read from Redpanda topics**: <<consuming-from-redpanda-topics>>
* **Transform or validate data**: <<data-transformation>>
* **Publish to Redpanda topics**: <<publishing-to-redpanda-topics>>
* **Process streaming data**: <<stream-processing-with-redpanda-connect>>
* **Build event-driven workflows**: <<event-driven-workflows>>
...
Outputs can include processors to transform, enrich, or validate data before publishing. This pattern is useful for adding timestamps, generating IDs, or ensuring data quality.

[source,yaml]
----
include::ai-agents:example$resources/outputs/redpanda-publish-with-processors.yaml[]
----

In this example:

* Processors add metadata (`published_at`, `source`, `event_id`) before publishing
* Validation ensures required fields exist
* The output publishes enriched data to Redpanda

This pattern keeps data transformation logic with the output, making tools more maintainable.

See also: xref:components:outputs/redpanda.adoc[`redpanda` output], xref:components:processors/mutation.adoc[`mutation` processor]
...
// Production workflows section - single-sourced from partials
// Part 1: Parameter validation, dynamic config, error handling, conditional processing
include::ai-agents:partial$mcp/production-workflows-before-secrets.adoc[]

// Secrets section - deployment-specific for Connect (uses environment variables)
include::ai-agents:partial$mcp/secrets-connect.adoc[]

// Part 2: Monitoring, multi-step enrichment, workflow orchestration
include::ai-agents:partial$mcp/production-workflows-after-secrets.adoc[]
