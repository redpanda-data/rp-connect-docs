...
* **Data pipeline orchestration**: Build tools that consume from Kafka topics, transform data, and write to destinations—let AI orchestrate complex pipelines
* **API integrations**: Create tools that call internal APIs, third-party services, or webhooks—AI handles the request formatting and error handling
* **Database operations**: Add tools for querying, inserting, or updating data—natural language becomes your query interface
* **Monitoring and observability**: Expose tools that check system health, query metrics, or analyze logs—ask AI to investigate issues
* **Multi-step workflows**: Combine multiple tools with different tags—AI chains them together based on your requirements

See xref:ai-agents:mcp-server/developer-guide.adoc[] to learn advanced patterns for building MCP tools.

View the xref:components:about.adoc[full catalog of Redpanda Connect components] you can use to build tools.
