...
ifdef::env-cloud[]
For the complete field reference, see xref:redpanda-adp:mcp/remote/create-tool.adoc#mcp-metadata[MCP metadata fields].
endif::[]
ifndef::env-cloud[]
...
ifdef::env-cloud[]
For patterns that apply defaults and validate values at runtime, see xref:redpanda-adp:mcp/remote/tool-patterns.adoc#input-validation[input validation patterns].
endif::[]
ifndef::env-cloud[]
