...
[WARNING]
====
Changing or removing `checkpoint_namespace` on an existing pipeline changes its checkpoint key, so the pipeline finds no prior checkpoint and resumes from `start_from`. With `start_from: trim_horizon`, this replays the entire stream.
====
