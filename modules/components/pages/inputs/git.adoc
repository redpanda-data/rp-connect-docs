...
- `git_mime_type`
- `git_is_binary`
- `git_encoding` (when a source file is base64 encoded)
- `git_deleted` (when a source file is deleted)

You can access these metadata fields using using xref:configuration:interpolation.adoc#bloblang-queries[function interpolation].
