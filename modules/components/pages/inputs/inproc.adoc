...
:categories: ["Utility"]
// tag::meta[]
:description: Directly connects to an output within the same Redpanda Connect process by a chosen ID, to link isolated streams when running in streams mode.
// end::meta[]
...
```yml
include::components:example$common/inputs/inproc.yaml[]
```
