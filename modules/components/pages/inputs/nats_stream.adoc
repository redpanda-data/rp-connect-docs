...
When a consumer closes its connection it unsubscribes. When all consumers of a durable queue do this the offsets are deleted. To stop consumers from unsubscribing, set the field `unsubscribe_on_close` to `false`.
...
EXPERIMENTAL: A xref:guides:bloblang/about.adoc[Bloblang mapping] that attempts to extract an object containing tracing propagation information, which is then used as the root tracing span for the message. The specification of the extracted fields must match the format used by the service wide tracer.
