...
Sends encrypted messages from Apache Kafka to Redpanda, using an https://docs.ockam.io/portals/kafka/redpanda[Ockam Portal^].
...
A list of topics to consume from. You can list multiple comma-separated topics in a single element. If you specify a `consumer_group`, partitions are automatically distributed across consumers of a topic. Otherwise, all partitions are consumed.
...
Use only TLS for message encryption.
