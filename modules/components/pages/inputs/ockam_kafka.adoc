...
Uses https://docs.ockam.io/[Ockam^] to produce encrypted messages from external Kafka topics. You can consume the encrypted messages using the `ockam_kafka` output or by https://command.ockam.io/manual/ockam-kafka-inlet-create.html[creating a Kafka Portal Inlet^] using Ockam Command.
...
For example, `orders_producer` forces the producer to present an Ockam credential, which confirms that the producer has the attribute `orders_producer=true`.
