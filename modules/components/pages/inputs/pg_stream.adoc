...
For this input to make a successful connection to your database, also make sure that your database allows replication connections.
...
When you run a pipeline that uses the `pg_stream` input, Redpanda Connect makes a connection to your PostgreSQL database and creates a replication slot. The replication slot uses a copy of the Write-Ahead Log (WAL) file to subscribe to changes in your database records as they are applied to the database itself.
...
When set to `true`, all data updates in a transaction are batched into a single message.
