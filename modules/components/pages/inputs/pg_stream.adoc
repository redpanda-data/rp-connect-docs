...
You can choose between two replication modes. If you set the <<stream_snapshot,`stream_snapshot` field>> to `true`, Redpanda Connect:

. Creates a snapshot of your database.
. Streams the contents of the tables specified in the `pg_stream` input.
