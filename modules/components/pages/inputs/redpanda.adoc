...
This input emits a `redpanda_lag` metric with `topic` and `partition` labels for each consumed topic, which records the difference (lag) between produced messages and messages read by the current consumer group.
