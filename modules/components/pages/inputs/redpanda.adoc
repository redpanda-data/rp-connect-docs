...
Use this connector in conjunction with the xref:components:outputs/redpanda.adoc[`redpanda` output], if you need to move topic data between Redpanda clusters, or other Apache Kafka clusters.

NOTE: If you need to read data from one or more topics on a Redpanda cluster, and then write the data to another topic on the same cluster, consider using the xref:components:inputs/redpanda_common.adoc[`redpanda_common` input] and xref:components:outputs/redpanda_common.adoc[output] instead.
...
For example, the following configuration includes a `fallback` output that writes delivery errors into a dead letter queue topic (`foo_dlq`), which is also retried indefinitely as a way to apply back pressure.
