= schema_registry
// tag::single-source[]
:type: input
:commercial-names: Schema Registry
:categories: ["Integration"]
// tag::meta[]
:description: Reads schemas from SchemaRegistry.
// end::meta[]

// © 2024 Redpanda Data Inc.


component_type_dropdown::[]


Reads schemas from a schema registry. You can use this connector to extract and back up schemas during a data migration. This input uses the https://github.com/twmb/franz-go/tree/master/pkg/sr[Franz Kafka Schema Registry client^].

ifndef::env-cloud[]
Introduced in version 4.32.2.
endif::[]

[tabs]
======
Common::
+
--

```yml
include::components:example$common/inputs/schema_registry.yaml[]
```

--
Advanced::
+
--

```yml
include::components:example$advanced/inputs/schema_registry.yaml[]
```

--
======

include::connect:components:partial$metadata/inputs/schema_registry.adoc[]



== Example

This example reads all schemas from a schema registry that are associated with subjects matching the `^foo.*` filter, including deleted schemas. 

```yaml
input:
  schema_registry:
    url: http://localhost:8081
    include_deleted: true
    subject_filter: ^foo.*
```

include::connect:components:partial$fields/inputs/schema_registry.adoc[]

// end::single-source[]