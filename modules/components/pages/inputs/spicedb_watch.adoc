...
You must use a xref:configuration:resources.adoc[cache resource] to store the https://authzed.com/docs/spicedb/concepts/consistency#zedtokens[ZedToken^] (ID) of the latest message consumed and acknowledged by this input. Ideally, the cache should persistent across restarts. This means that every time the input is initialized, it starts reading from the newest data updates.
...
The xref:configuration:resources.adoc[cache resource] to use for performing unread message backfills. The ZedToken (ID) of the last message received must be stored in this cache, which is then used for subsequent requests.
