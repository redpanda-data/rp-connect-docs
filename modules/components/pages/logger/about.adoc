= Logger
// tag::meta[]
:description: Learn how to configure Redpanda Connect logging, including log formats and levels.
// end::meta[]
...
ifndef::env-cloud[]
TIP: You can also configure the xref:components:redpanda/about.adoc[`redpanda` component] to send logs and status updates to topics on a Redpanda cluster. For monitoring pipeline health with status events, see xref:guides:monitor-pipeline-status.adoc[].
endif::[]
