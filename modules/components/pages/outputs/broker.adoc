...
A meta-output that routes messages to multiple child outputs simultaneously using a range of brokering <<patterns>>. Unlike regular outputs, `broker` doesn't send messages anywhere by itself. Instead, it wraps other outputs and controls how messages are delivered to each one.
