...
Writes a batch of messages to any graph database that supports the https://neo4j.com/docs/bolt/current/bolt/[Bolt protocol^] and the https://neo4j.com/docs/cypher-manual/current/introduction/[Cypher query language^]. For each incoming message, the connector can perform operations to store or delete data within the database using the Cypher query language.
...
+
--
This is an example of how to write data to Neo4j Aura.
```yaml
output:
  cypher:
