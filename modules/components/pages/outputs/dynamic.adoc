...
:categories: ["Utility"]
// tag::meta[]
:description: A special broker type where the outputs are identified by unique labels and can be created, changed and removed during runtime via a REST API.
// end::meta[]
...
```yml
include::components:example$common/outputs/dynamic.yaml[]
```

The broker pattern used is always `fan_out`, meaning each message will be delivered to each dynamic output.

include::connect:components:partial$fields/outputs/dynamic.adoc[]
