...
NOTE: The underlying library for this output has been deprecated. The xref:components:outputs/elasticsearchv8.adoc[`elasticsearch_v8` output] offers an up-to-date alternative based on the the https://github.com/elastic/go-elasticsearch?tab=readme-ov-file[go-elasticsearch/v8] library. For full information about breaking changes and version compatibility, see https://www.elastic.co/guide/en/elasticsearch/reference/current/migrating-8.0.html[Elastic's Migrating to 8.0 guide^]. 
