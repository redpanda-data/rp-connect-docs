...
NOTE: The xref:components:outputs/elasticsearch_v8.adoc[`elasticsearch_v8` output] is based on the the https://github.com/elastic/go-elasticsearch?tab=readme-ov-file[go-elasticsearch/v8] library. For full information about breaking changes and version compatibility, see https://www.elastic.co/guide/en/elasticsearch/reference/current/migrating-8.0.html[Elastic's Migrating to 8.0 guide^]. 
...
You can use xref:configuration:interpolation.adoc#bloblang-queries[function interpolations] to dynamically set values for the <<id, `id`>> and <<index, `index`>> fields. When message batches are sent, interpolations are performed per message part.

== Fields

=== `urls`

A list of URLs to connect to. This output attempts to connect to each URL in the list, in order, until a successful connection is established. The output then continues to use that URL until the connection is closed.
...
output:
  ...
  # Sets the document ID to the value of the metadata ID field. 
    id: ${! meta("id") }
```
