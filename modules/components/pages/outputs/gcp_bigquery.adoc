...
You can use the xref:components:processors/parquet_encode.adoc[`parquet_encode` processor] to convert message data into a Parquet format before it is sent to this output. Each message sent by the processor must be a full Parquet message.
