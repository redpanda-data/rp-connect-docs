...
ifndef::env-cloud[]
For tested configurations that tune this output for high throughput, including the trade-off between preserving message ordering and maximizing throughput, see xref:guides:iceberg-output-tuning.adoc[Tune the Iceberg output for throughput].
endif::[]
