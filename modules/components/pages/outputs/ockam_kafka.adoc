...
Uses https://docs.ockam.io/[Ockam^] to consume encrypted messages and writes them to a Kafka topic. You can also produce encrypted messages using the `ockam_kafka` input or by https://command.ockam.io/manual/ockam-kafka-inlet-create.html[creating a Kafka Portal Inlet^] using Ockam Command.

Ockam’s encryption method guarantees that downstream consumers receive topic data from authenticated producers that has not been tampered with or leaked in-flight. The Kafka brokers or service providers which messages pass through cannot see or manipulate message data.
...
Specify one or more methods or mechanisms of SASL authentication (optional). Authentication mechanisms are tried in order. If the broker supports the first SASL mechanism, all connections use it. If the first mechanism fails, the client picks the first supported mechanism. If the broker does not support any client mechanisms, all connections fail.
...
* Only message payloads remain unencrypted. It does not disable TLS or any other transport-layer encryption that may also be enabled.
* All participants in a topic must also have their settings set to `true`.
...
For example, `orders_consumer` forces the consumer to present an Ockam credential, which confirms that the consumer has the attribute `orders_consumer=true`.
