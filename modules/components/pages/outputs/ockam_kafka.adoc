...
The topic to write messages to (required). This field supports xref:configuration:interpolation.adoc#bloblang-queries[interpolation functions].
...
Specify an ACL policy to validate against the Kafka consumer.

For example, `(= subject.kafka_role "consumer")` verifies that the Kafka consumer has a `kafka_role` attribute which contains the value `true`.
...
The TCP listening address of the Kafka consumer.
