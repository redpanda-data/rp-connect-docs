...
IMPORTANT: Redpanda recommends that you enable the dedupe feature on the QuestDB server. For more information about about deploying, configuring, and using QuestDB, see the https://questdb.io/docs/[QuestDB documentation]. 
...
This output can also send message batches. You can configure batches at both the input and output level. For more information, see xref:configuration:batching.adoc[Message Batching].
...
The address of the HTTP port on the QuestDB server, excluding the HTTP protocol.
