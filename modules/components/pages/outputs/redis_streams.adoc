= redis_streams
// tag::single-source[]
:page-commercial-names: Redis Streams, Redis
:type: output
:status: stable
:categories: ["Services"]
...
```yml
include::components:example$common/outputs/redis_streams.yaml[]
```
...
```yml
include::components:example$advanced/outputs/redis_streams.yaml[]
```
...
include::connect:components:partial$fields/outputs/redis_streams.adoc[]
