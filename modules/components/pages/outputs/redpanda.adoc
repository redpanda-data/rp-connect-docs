...
Grants an additional buffer or overhead to requests that have timeout fields defined. This field is based on the behavior of `request.timeout.ms`, but with the option to extend the timeout deadline.
