...
Use this connector in conjunction with the xref:components:inputs/redpanda_common.adoc[`redpanda_common` input], to read data from one or more topics on a Redpanda cluster, and then write the data to another topic on the same cluster.

To avoid duplicating Redpanda cluster credentials in your `redpanda_common` input, output, or any other components in your data pipeline, you can use a single `redpanda` configuration block. For more details, see the <<pipeline-example,Pipeline example>>.
...
This data pipeline reads data from `topic_A` on a Redpanda cluster, transforms the data, and then writes it to `topic_B` on the same cluster. The cluster details are configured within the `redpanda` configuration block, so you only need to configure them once. This is a useful feature when you have multiple inputs and outputs in the same data pipeline that need to connect to the same cluster.
