...
| TIME, DATE, TIMESTAMP 
| `timestamp`, or `number` where the `number` is a converted to a Unix timestamp
...
| The time taken to build the output file that is sent to Snowflake.
...
NOTE: `snowflake_convert_latency_ns` + `snowflake_serialize_latency_ns` = `snowflake_build_output_latency_ns`
