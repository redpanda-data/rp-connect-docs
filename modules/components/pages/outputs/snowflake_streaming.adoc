...
The conversion of message data to Snowflake table rows is determined by the: 
...
NOTE: For reduced complexity, consider <<schema_evolution, enabling schema evolution>>, which automatically creates and updates the Snowflake table schema based on message contents.

=== Scenario: Data and table schema match (schema evolution enabled or disabled) 
...
- The message values match the column data types. (If there was a data mismatch, an error would be logged.)
...
=== Scenario: Data and table schema mismatch (schema evolution enabled) 

An output message includes schema updates and the `schema_evolution.enabled` field is set to `true`.

The target Snowflake table has the same two columns as for the <<scenario-data-and-table-schema-match-schema-evolution-enabled-or-disabled, previous scenario>>:
...
=== Scenario: Data and table schema mismatch (schema evolution disabled)
...
The pipeline generates the same message as for the <<scenario-data-and-table-schema-mismatch-schema-evolution-enabled,previous scenario>>:
...
- As schema evolution is disabled, Redpanda Connect ignores the mismatched column names and values and inserts a row of null values.
+
|===
| product_id | product_code
...
Whether schema evolution is enabled. When set to `true`, the Snowflake table is automatically created based on the schema of the first message written to it, if the table does not already exist. As new fields are added to subsequent messages in the pipeline, new columns are created in the Snowflake table.
...
ifndef::env-cloud[]
NOTE: Versions 4.48.1 and earlier do not include this setting. New columns with `null` values were automatically added to the Snowflake table when schema evolution was enabled.
endif::[]
