...
Options to control schema updates as new tables and columns are added to the pipeline.

*Type*: `object`

=== `schema_evolution.enabled`

Whether schema evolution is enabled. Set to `true` to allow the automatic creation of new tables and columns in the <<schema,specified schema>> when they are added to your pipeline.
