...
== Column mapping between Redpanda Connect and Snowflake

The column mapping behavior of the `snowflake_streaming` output is determined by: 

- The output message contents.
- The <<schema_evolution, schema evolution settings>>.
- The schema of the <<table, target Snowflake table>>. 
...
- The message values match the column data types.
...
The target Snowflake table has the same two columns as scenario one:
...
- As schema evolution is enabled, Redpanda Connect adds two new columns to the target table with data types derived from the output message values. 
...
The pipeline generates the same message as for scenario two:
...
- As schema evolution is disabled, Redpanda Connect ignores the new column names and inserts a row with null values.
+
|===
| product_id | product_code
