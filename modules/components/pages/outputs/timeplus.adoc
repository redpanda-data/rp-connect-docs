...
Sends messages to a data stream on https://docs.timeplus.com/[Timeplus Enterprise (Cloud or Self-Hosted)^] using the https://docs.timeplus.com/ingest-api[Ingest API^], or directly to the `timeplusd` server process.
...
If your upstream data source or pipeline returns unstructured messages, such as strings, you can configure a output processor to transform the messages. See the <<unstructured-message,Unstructured message>> section for examples.
...
- `<api-key>`: The API key generated in the web console of Timeplus Enterprise (Cloud).
...
You must specify the HTTP port for the `timeplusd` server process.
...
If your upstream data source or pipeline returns unstructured messages, such as strings, you can configure an output processor to transform them into a structured messages and then pass them to the output.

This example creates structured messages using the `raw` field, and stores the original string content into it. You can modify the name of the field, but you must also add either the the `raw` field name or your custom field name to the destination data stream.
...
The destination platform. For Timeplus Enterprise (Cloud or Self-Hosted), enter `timeplus`, or `timeplusd` for a `timeplusd` server process.
...
The API key generated in the web console of Timeplus Enterprise (Cloud). This field is required if you are sending message to Timeplus Enterprise (Cloud).
...
The username for the Timeplus application server. This field is required if you are sending message to Timeplus Enterprise (Self-Hosted) or `timeplusd`.

*Type*: `string`

=== `password`

The password for the Timeplus application server. This field is required if you are sending message to Timeplus Enterprise (Self-Hosted) or `timeplusd`.
