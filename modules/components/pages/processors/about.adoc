= Processors
// tag::meta[]
:description: Learn how processors work in Redpanda Connect. Processors are functions that mutate, filter, or enrich messages passing through a pipeline.
// end::meta[]
// tag::single-source[]
...
All Redpanda Connect processors support multiple-part messages, which are synonymous with batches. This enables xref:configuration:windowed_processing.adoc[windowed processing] capabilities.
