= aws_dynamodb_partiql
// tag::single-source[]
:page-commercial-names: Amazon, AWS, DynamoDB PartiQL
:type: processor
:status: experimental
:categories: ["Integration"]

// © 2024 Redpanda Data Inc.


component_type_dropdown::[]


Executes a PartiQL expression against a DynamoDB table for each message.

ifndef::env-cloud[]
Introduced in version 3.48.0.
endif::[]

[tabs]
======
Common::
+
--

```yml
include::components:example$common/processors/aws_dynamodb_partiql.yaml[]
```

--
Advanced::
+
--

```yml
include::components:example$advanced/processors/aws_dynamodb_partiql.yaml[]
```

--
======

Both writes or reads are supported, when the query is a read the contents of the message will be replaced with the result. This processor is more efficient when messages are pre-batched as the whole batch will be executed in a single call.

include::connect:components:partial$examples/processors/aws_dynamodb_partiql.adoc[]

include::connect:components:partial$fields/processors/aws_dynamodb_partiql.adoc[]

// end::single-source[]