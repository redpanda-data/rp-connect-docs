= gcp_vertex_ai_chat
// tag::single-source[]
:page-commercial-names: Google Cloud, GCP
:type: processor
:categories: ["AI"]
// tag::meta[]
:description: Generates responses to messages in a chat conversation, using the Vertex AI API.
// end::meta[]


// © 2024 Redpanda Data Inc.


component_type_dropdown::[]

Generates responses to messages in a chat conversation, using the https://cloud.google.com/vertex-ai/docs/start/introduction-unified-platform[Vertex API AI^].

ifndef::env-cloud[]
Introduced in version 4.34.0.
endif::[]

[tabs]
======
Common::
+
--

```yml
include::components:example$common/processors/gcp_vertex_ai_chat.yaml[]
```

--
Advanced::
+
--

```yml
include::components:example$advanced/processors/gcp_vertex_ai_chat.yaml[]
```

--
======

This processor sends prompts to your chosen large language model (LLM) and generates text from the responses, using the Vertex AI API.

For more information, see the https://cloud.google.com/vertex-ai/docs[Vertex AI documentation^].

include::connect:components:partial$fields/processors/gcp_vertex_ai_chat.adoc[]

// end::single-source[]