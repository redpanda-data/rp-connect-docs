...
HTTP status codes in the 300-399 range are redirects. The <<follow_redirects,`follow_redirects` field>> allows you to choose how to handle these responses.

When a request returns a response code within the `backoff_on` field, it is retried after increasing intervals.

When a request returns a response code within the `drop_on` field, it is immediately considered a failed request.
...
Whether to follow redirects, which includes all responses with HTTP status codes in the 300-399 range. If set to `false`, the response message only contains the body, status, and headers from the redirect response, and this processor does not make a request to the URL set in the Location header of the response. 
