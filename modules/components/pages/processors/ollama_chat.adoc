...
The external tools the LLM can invoke, such as functions, APIs, or web browsing. You can build sub-pipelines that include definitions of these tools, and the LLM chooses when to invoke them to help answer a prompt. For more details, see <<Examples, examples>>.
...
A description of what the tool does, which the LLM uses to decide when to invoke it.
...
Use a sub-pipeline to make calls to external tools::
+
--
In this configuration, the LLama 3.2 model executes a sub-pipeline, which makes a tool call to retrieve weather data for a specific city.
          ...
          - name: GetWeather
            ...
            processors:
              - http:
                  ...
                  headers:
                    # Uses the curl user-agent to return a text string from the weather report website
