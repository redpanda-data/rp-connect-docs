...
The external tools the model can invoke, such as functions, APIs, or web browsing. You can build a series of processors that include definitions of these tools, and the specified model can choose when to invoke them to help answer a prompt. For more information, see <<Examples, Examples>>.
...
Sets this parameter to a enum, and defines the specific set of values the model must use.
...
+
--
This configuration fetches image URLs from stdin and uses the GPT-4o LLM to describe the images.
