...
mapping: |
  ...
  map isDebeziumMicroTimestampType {
    root = this.type == "long" && this."connect.name" == "io.debezium.time.MicroTimestamp" && !this.exists("logicalType")
  }
  map isDebeziumZonedTimestampType {
    root = this.type == "string" && this."connect.name" == "io.debezium.time.ZonedTimestamp" && !this.exists("logicalType")
  }
  map debeziumAddLogicalTypes {
    let mapped_fields = this.fields.or([]).map_each(item -> item.apply("debeziumAddLogicalTypes"))
    root = match {
      this.type == "record" => this.assign({"fields": $mapped_fields})
      this.type == "long" && this.apply("isDebeziumTimestampType") => this.assign({"logicalType":"timestamp-millis"})
      this.type == "long" && this.apply("isDebeziumMicroTimestampType") => this.assign({"logicalType":"timestamp-micros"})
      this.type == "string" && this.apply("isDebeziumZonedTimestampType") => this.assign({"logicalType":"timestamp-millis"})
      this.type.type() == "array" => this.assign({"type": this.type.map_each(item -> item.apply("debeziumAddLogicalTypes"))})
      this.type.type() == "object" && this.type.apply("isDebeziumTimestampType") => this.merge({"type":{"logicalType":"timestamp-millis"}})
      this.type.type() == "object" && this.type.apply("isDebeziumMicroTimestampType") => this.merge({"type":{"logicalType":"timestamp-micros"}})
      this.type.type() == "object" && this.type.apply("isDebeziumZonedTimestampType") => this.merge({"type":{"logicalType":"timestamp-millis"}})
      _ => this
    }
  }
  root = this.apply("debeziumAddLogicalTypes")
```
