...
NOTE: Because the failure flag is cleared before the `catch` processors run, the xref:guides:bloblang/functions.adoc#error[`error`] and `error_source_*` functions do not report the original failure within the `catch` block; use the metadata object instead. An empty or omitted `catch` simply records the error in metadata and clears the flag (the failure is swallowed).
...
```yml
include::components:example$common/processors/try_catch.yaml[]
```
...
```yml
include::components:example$advanced/processors/try_catch.yaml[]
```

--
======

include::connect:components:partial$fields/processors/try_catch.adoc[]
