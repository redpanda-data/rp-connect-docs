...
As well as the xref:components:logger/about.adoc[default logger], you can configure Redpanda Connect to send logs and status updates to topics on a Redpanda cluster.

The `redpanda` namespace contains the configuration of the logging server, which uses the following default values:
...
The topic status updates are sent to.
...
The root certificate authority to use (optional). This is a string, representing a certificate chain from the parent trusted root certificate, to possible intermediate signing certificates, to the host certificate.
...
The path of the root certificate authority file to use (optional). This is a file, often with a `.pem` extension, containing a certificate chain from the parent trusted root certificate, to possible intermediate signing certificates, to the host certificate.
