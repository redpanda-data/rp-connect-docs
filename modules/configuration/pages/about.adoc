...
----
{binary-name} -r ./staging/request.yaml -c ./config.yaml
----
...
[,bash,subs="attributes+"]
----
{binary-name} -r ./production/request.yaml -c ./config.yaml
----

These flags also support wildcards, which allows you to import an entire directory of resource files like `{binary-name} -r "./staging/*.yaml" -c ./config.yaml`. You can find out more about configuration resources in the xref:configuration:resources.adoc[resources document].
...
But hey, why don't you chill out? {page-component-title} has a (currently experimental) alternative feature called templates, with which it's possible to define a custom configuration schema and a template for building a configuration from that schema. You can read more about templates xref:configuration:templating.adoc[in this guide].
...
----
# Normal mode
{binary-name} -w -r ./production/request.yaml -c ./config.yaml
----

[,bash,subs="attributes+"]
----
# Streams mode
{binary-name} -w -r ./production/request.yaml streams ./stream_configs/*.yaml
----
...
[,bash,subs="attributes+"]
----
{binary-name} create websocket/mapping/kafka
----

____
If you need a gentle reminder as to which components {page-component-title} offers you can see those as well with `{binary-name} list`.
____
...
For more information read the output from `{binary-name} create --help`.
...
[,bash,subs="attributes+"]
----
{binary-name} lint ./foo.yaml
./foo.yaml: line 3: field yourl not recognized
----

For more information read the output from `{binary-name} lint --help`.
...
[,bash,subs="attributes+"]
----
{binary-name} -c ./your-config.yaml echo
----
