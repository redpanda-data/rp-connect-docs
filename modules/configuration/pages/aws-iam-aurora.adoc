    ...
    "Statement": [
        {
            "Effect": "Allow",
            "Principal": {
                "AWS": "arn:aws:iam::<RP_ACCT_ID>:role/redpanda-<CLUSTER_ID>-redpanda-connect-pipeline"
            },
...
Replace `<RP_ACCT_ID>` with the Redpanda AWS account ID.
...
In the Redpanda AWS account, add an inline policy to the `redpanda-<CLUSTER_ID>-redpanda-connect-pipeline` role to allow it to assume your database connect role:
...
aws iam put-role-policy \
  --role-name "redpanda-<CLUSTER_ID>-redpanda-connect-pipeline" \
