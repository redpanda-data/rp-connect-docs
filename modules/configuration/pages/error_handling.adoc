...
Finally, you can attach additional metadata when routing to the DLQ, such as the error message. This can be done by running a series of xref:components:processors/about.adoc[processors] before sending the data to the final xref:components:outputs/about.adoc[output].
...
output:
  fallback:
    ...
    - processors:
        - mutation: |
            root.error = @fallback_error # Add the error message to the message we're sending to the DLQ output
      resource: processor_2 # Only errored messages, or delivery failures to processor_1, go here
----
