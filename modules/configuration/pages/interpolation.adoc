= Interpolation
// tag::meta[]
:description: Learn how to use environment variables and function interpolations to insert dynamic values into Redpanda Connect configurations.
// end::meta[]
// tag::single-source[]

ifdef::env-cloud[]
NOTE: Environment variables are not currently supported in Redpanda Connect in Redpanda Cloud, but you can use xref:cloud-data-platform:develop:connect/configuration/contextual-variables.adoc[contextual variables] to access information about the environment in which a data pipeline is running, and the pipeline itself.
endif::[]
