= Resources
// tag::meta[]
:description: Learn how to declare components as named resources in Redpanda Connect so one configured instance can be referenced throughout a configuration.
// end::meta[]
// tag::single-source[]
...
Then when you execute Redpanda Connect use the environment variable to choose your resource: `FEATURE_REQUEST=get_foo rpk connect run ./your_config.yaml`.

=== With imports

However, Redpanda Connect allows you to import resources from separate files with the cli flag `-r` or `-resources`, which can be a useful way to switch out resources with common names based on your chosen environment. For example, with a main configuration file `config.yaml`:
