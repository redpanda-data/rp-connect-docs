...
Then when you execute {page-component-title} use the environment variable to choose your resource: `FEATURE_REQUEST=get_foo {binary-name} -c ./your_config.yaml`.
...
These flags also support wildcards, which allows you to import an entire directory of resource files like `{binary-name} -r "./staging/*.yaml" -c ./config.yaml`.
