...
From version 4.39.0, you can use the `rpk connect` CLI flag `--secrets` to look up secrets values on a remote system at runtime. For example, in your secrets management solution. This means that Redpanda Connect resolves interpolations in your configuration at runtime without setting environment variables.
