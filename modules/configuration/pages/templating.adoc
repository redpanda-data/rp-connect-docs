...
Required when `fields[].type` is `string_enum` or `string_annotated_enum`.

- For `string_enum`, provide a non-empty array of string values.
- For `string_annotated_enum`, provide a map of `value: "description"` pairs used to annotate behaviour in the docs UI.
