...
:description: Learn how to use {page-component-title} to create a vanity chat bot.

Stream processing is stupid and boring, and so it's important to re-purpose tools like {page-component-title} for fun things occasionally. This cookbook outlines how {page-component-title} can be used to create a Discord bot for important tasks such as providing insults and bad jokes to your chat. If you're a member of the https://discord.gg/6VaWjzP[{page-component-title} Discord server] then you're likely already familiar with Blob Bot which is the resulting product.
...
----
{binary-name} -e testing.env -c ./config.yaml
...
____
We're hitting the Github API with the xref:components:processors/http.adoc[generic `http` processor], which can be configured to work with most HTTP based APIs. In fact, the Discord input and output are actually xref:configuration:templating.adoc[configuration templates] that use the generic HTTP components https://github.com/benthosdev/benthos/blob/master/template/outputs/discord.yaml[under the hood].
____
...
The full config for Blob Bot (with some super secret responses redacted) can be found https://github.com/benthosdev/benthos/blob/master/config/examples/discord_bot.yaml[in the Github repo]. To find out more about Bloblang check out xref:guides:bloblang/about.adoc[the guide page]. To find out more about config templates check out the xref:configuration:templating.adoc[templates documentation page].
