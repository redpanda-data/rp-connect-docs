...
:learning-objective-1: Apply reusable patterns for capturing DynamoDB CDC events
...
The examples in this cookbook use environment variables for AWS configuration. Environment variables keep credentials out of your pipeline configuration files.

[,bash]
----
export DYNAMODB_TABLE=orders <1>
export AWS_REGION=us-east-1 <2>
export REDPANDA_BROKERS=localhost:9092 <3>
export S3_BUCKET=cdc-archive <4>
----
...
Filter events to process only specific change types:
...
[source,yaml]
----
include::cookbooks:example$dynamodb_cdc/to-redpanda.yaml[tag=config]
----
...
- Supports specialized downstream consumers
...
input:
  aws_dynamodb_cdc:
    table: orders
    checkpoint_table: cdc-checkpoints <1>
...
- Consider using https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/kds.html[Kinesis Data Streams for DynamoDB^] with the xref:components:inputs/aws_kinesis.adoc[`aws_kinesis` input] instead (up to 1 year retention)
