...
:description: How to configure {page-component-title} to process a workflow of enrichment services.
...
If we configure our enrichments within a xref:components:processors/workflow.adoc[`workflow` processor] we can use {page-component-title} to automatically detect our dependency graph, giving us two key benefits:

. Enrichments at the same level of a dependency graph (claims and hyperbole) will be executed in parallel.
. When introducing more enrichments to our pipeline the added complexity of resolving the dependency graph is handled automatically by {page-component-title}.
