...
1. Increase `max_retries` in the Jira processor config
2. Reduce query frequency by increasing `interval`
...
1. Use more specific JQL queries
2. Limit fields with the `fields` parameter
3. Reduce `max_results_per_page` if memory is constrained
