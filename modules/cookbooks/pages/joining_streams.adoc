...
There are xref:configuration:error_handling.adoc[many patterns for error handling] to choose from in {page-component-title}. In this example we're going to introduce a delayed retry queue as it enables us to reprocess failed documents after a grace period, which is isolated from our main pipeline.
...
You can find a full example https://github.com/benthosdev/benthos/blob/master/config/examples/joining_streams.yaml[in the project repo], and with this config we can deploy as many instances of {page-component-title} as we need as the partitions will be balanced across the consumers.
