...
By default, the `microsoft_sql_server_cdc` input creates a table (`rpcn.CdcCheckpointCache`) and stored procedure in your SQL Server database to store the last processed Log Sequence Number (LSN). This requires the pipeline user to have `CREATE TABLE` and `CREATE PROCEDURE` permissions.
...
export S3_BUCKET=my-cdc-bucket <3>
----
...
- Uses `schema.table` as the Redpanda topic name
...
- Organizes files by schema, table, and time-based partitions (year/month/day/hour)
...
- Routes `insert` events to one Redpanda topic and `update` and `delete` events to another
