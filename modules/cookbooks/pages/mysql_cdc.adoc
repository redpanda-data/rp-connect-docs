...
The `mysql_cdc` input captures row-level changes from MySQL tables using the binary log (binlog). This cookbook provides reusable patterns for filtering, transforming, and routing MySQL CDC events to Redpanda, S3, and other destinations.
...
Both variables must be set to `ON` and `ROW` respectively. For cloud-managed MySQL, see:
...
cache_resources:
  - label: redis_cache
    redis:
      url: ${REDIS_URL}
----
<1> Set to `false` to skip the snapshot and stream only new changes.
