...
*** xref:components:inputs/microsoft_sql_server_cdc.adoc#checkpoint_cache_table_name['checkpoint_cache_table_name'] - The multipart identifier for the checkpoint cache table name.
*** xref:components:inputs/microsoft_sql_server_cdc.adoc#checkpoint_cache_key['checkpoint_cache_key'] - The key to use to store the snapshot position in `checkpoint_cache`.
** xref:components:inputs/redpanda_migrator.adoc['redpanda_migrator']: xref:components:inputs/redpanda_migrator.adoc#schema_registry['schema_registry'] - Configuration for schema registry integration. Enables migration of schema subjects, versions, and compatibility settings between clusters.
* Outputs components
...
*** xref:components:outputs/redpanda_migrator.adoc#consumer_groups['consumer_groups']
...
- `redpanda_migrator_bundle` input and output (now deprecated; these are part of the legacy migration architecture and internally depend on the deprecated individual components)
...
Users should migrate from both the bundle and individual legacy component approaches to the new unified `redpanda_migrator` input/output pair. The migration guide provides detailed instructions for migrating from all legacy migration architectures.

For detailed migration instructions, see xref:guides:migration/migrate-unified-redpanda-migrator.adoc[].
