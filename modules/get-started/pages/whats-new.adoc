...
* **`metadata_max_age`** (Kafka/Redpanda inputs and outputs): The default changed from `5m` to `1m` to provide more responsive topic and partition discovery. This means metadata refreshes occur more frequently by default. If you experience increased broker load or want to maintain the previous behavior, explicitly set this field to `5m`.
...
* Bloblang-functions:
** xref:guides:bloblang/functions.adoc#bytes[`bytes`] (stable): Create a new byte array that is zero initialized
* Bloblang-methods:
** xref:guides:bloblang/methods.adoc#repeat[`repeat`] (stable)
