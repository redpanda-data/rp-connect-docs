...
* `redpanda` (xref:components:inputs/redpanda.adoc[input] and xref:components:inputs/redpanda.adoc[output]): Consume message data from or send message data to Kafka brokers.
...
* Create an xref:configuration:allow_and_deny_lists.adoc[allow list or deny list] for a Redpanda Connect instance (Self-Managed only)
