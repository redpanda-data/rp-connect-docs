...
This update includes a beta release of the xref:components:inputs/postgres_cdc.adoc[`postgres_cdc` input] for Redpanda Connect Self-Managed and Cloud, which captures data changes made to a PostgreSQL database in real time and streams them to Redpanda Connect.
