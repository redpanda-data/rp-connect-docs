...
This release renames the `pg_stream` input to xref:components:inputs/postgres_cdc.adoc[`postgres_cdc`]. The `postgres_cdc` input no longer emits `mode` metadata, but sets the `operation` metadata to `read` when a snapshot of a database is processed.
