...
* Inputs:
** xref:components:inputs/legacy_redpanda_migrator.adoc[`legacy_redpanda_migrator`] (deprecated): 
...
** xref:components:inputs/legacy_redpanda_migrator_offsets.adoc[`legacy_redpanda_migrator_offsets`] (deprecated): 
...
* Outputs:
** xref:components:outputs/legacy_redpanda_migrator.adoc[`legacy_redpanda_migrator`] (deprecated): 
...
** xref:components:outputs/legacy_redpanda_migrator_offsets.adoc[`legacy_redpanda_migrator_offsets`] (deprecated): This output should be used in combination with the `legacy_redpanda_migrator_offsets` input
...
*** xref:components:inputs/microsoft_sql_server_cdc.adoc#checkpoint_cache_table_name['checkpoint_cache_table_name'] - The multipart identifier for the checkpoint cache table name. If no `checkpoint_cache` field is specified, this input will automatically create a table and stored procedure under the `rpcn` schema to act as a checkpoint cache. This table stores the latest processed Log Sequence Number (LSN) that has been successfully delivered, allowing Redpanda Connect to resume from that point upon restart rather than reconsume the entire change table.
*** xref:components:inputs/microsoft_sql_server_cdc.adoc#checkpoint_cache_key['checkpoint_cache_key'] - The key to use to store the snapshot position in `checkpoint_cache`. An alternative key can be provided if multiple CDC inputs share the same cache.
...
*** xref:components:outputs/redpanda_migrator.adoc#sync_topic_acls['sync_topic_acls'] - Whether to synchronise topic ACLs from source to destination cluster. ACLs are transformed safely: ALLOW WRITE permissions are excluded, and ALLOW ALL is downgraded to ALLOW READ to prevent conflicts.
*** xref:components:outputs/redpanda_migrator.adoc#serverless['serverless'] - Enable serverless mode for Redpanda Cloud serverless clusters. This restricts topic configurations and schema features to those supported by serverless environments.
