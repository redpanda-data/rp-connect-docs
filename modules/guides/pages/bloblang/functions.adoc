...
Creates a new byte array of a specified length that is zero-initialized. This function is useful when you need to allocate a fixed-size byte buffer for binary data operations.

==== Parameters

*`length`* &lt;integer&gt; The number of bytes to allocate in the array. Must be a non-negative integer.
...
Prepends a 5-byte Schema Registry header to a message. The header consists of a magic byte (0x00) followed by a 4-byte big-endian schema ID.

==== Parameters

- *`schema_id`* &lt;number&gt; The schema ID to encode in the header.
- *`payload`* &lt;string|bytes&gt; The message payload to which the header will be prepended.
