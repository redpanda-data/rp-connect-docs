...
Attempts to parse a value into a `timestamp`. You can also provide an optional argument, which is returned when the value cannot be parsed into a `timestamp`. The output of this method can be further transformed using <<timestamp-manipulation,timestamp manipulation methods>>.
