...
Returns a string consisting of the target string repeated a specified number of times. If the count is zero or negative, an empty string is returned.

==== Parameters

*`count`* &lt;integer&gt; The number of times to repeat the string. Must be an integer value.
...
Splits a string into segments using https://hexdocs.pm/unicode_string/readme.html#segmentation[Unicode text segmentation rules^].

==== Parameters

*`segmentation_type`* &lt;string&gt; The text segmentation rule to use, either: `"sentence"`, `"grapheme"`, or `"word"`.
...
This operation is often used for masking bits.
...
- If both are `0`, the result bit is `0`

This operation is useful for setting or combining flags.
...
Attempts to parse a value into a `timestamp`. The input value must be one of the following:
...
You can also provide an optional argument, which is returned when the value cannot be parsed into a `timestamp`. The output of this method can be further transformed using <<timestamp-manipulation,timestamp manipulation methods>>.
...
Attempts to infer the schema of a given value. The resulting schema can then be used as an input to schema conversion and enforcement methods.
