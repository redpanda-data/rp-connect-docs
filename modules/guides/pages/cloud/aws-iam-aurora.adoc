...
====
This guide applies to self-hosted Redpanda Connect deployments. Redpanda Cloud pipelines do not currently support IAM Roles for Service Accounts (IRSA) or cross-account `sts:AssumeRole` in Bring Your Own Cloud (BYOC) or Dedicated clusters. The Redpanda Connect binary supports both, but the Cloud platform does not currently make pod identity tokens available to pipelines, and you cannot grant additional permissions to Redpanda-managed IAM roles. To authenticate Cloud pipelines to AWS services, store static credentials as secrets and reference them in your pipeline configuration. See xref:cloud-data-platform:develop:connect/configuration/secret-management.adoc[Manage Secrets in Redpanda Cloud].
====
...
This page covers configuring both roles with the required trust relationships. You own both roles: the Connect role that your deployment runs as, and the database connect role in the Aurora account.
...
* On Amazon EKS, this is the IRSA role associated with the Kubernetes service account that runs your Redpanda Connect pods.
* On EC2, this is the role attached to the instance profile.
...
The Aurora security group must allow inbound traffic from your Redpanda Connect deployment's egress IP addresses on the database port (5432 for PostgreSQL, 3306 for MySQL). For EKS deployments, this is typically the NAT gateway or node IP range of the cluster's VPC.
