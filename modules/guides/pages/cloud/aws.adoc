...
[NOTE]
====
Redpanda Cloud pipelines cannot assume roles through the default credential chain (IAM Roles for Service Accounts), and you cannot grant Redpanda-managed IAM roles permission to assume your roles. To assume a role from a Cloud pipeline, set explicit `id` and `secret` credentials for an IAM identity that you own and that has permission to assume the target role, and make sure the target role's trust policy trusts that identity. Store these credentials as xref:cloud-data-platform:develop:connect/configuration/secret-management.adoc[secrets].
====
endif::[]
...
* xref:guides:cloud/aws-iam-aurora.adoc[Authenticate to Amazon Aurora using IAM roles]
