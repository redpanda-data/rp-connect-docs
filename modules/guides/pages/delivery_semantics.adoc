...
* Messages are not lost unless explicitly dropped in configuration
* Ensures every message is processed at-least-once, with optional retry logic for failures
* Duplicates may occur in failure recovery scenarios, which is typical of at-least-once systems
...
* Error-handling components (for example, `try`, or `catch`) allow users to:
