...
For Linux and MacOS, you can use Redpanda Connect with Redpanda Keeper (`rpk`). The `rpk` command-line tool allows you create and manage data pipelines with Redpanda Connect as well as xref:ROOT:get-started:rpk-install.adoc[interact with Redpanda clusters].
...
[source,sh,subs="attributes+"]
----
docker run --rm ghcr.io/{project-github} create > ./config.yaml
----
...
[source,sh,subs="attributes+"]
----
docker run --rm -v $(pwd)/config.yaml:/connect.yaml ghcr.io/{project-github}
----
