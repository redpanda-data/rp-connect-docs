...
NOTE: The `rpk` CLI also includes an `rpk connect` plugin, which manages installations and upgrades of Redpanda Connect. This plugin is automatically installed when you start to run `rpk connect` commands, unless you run `rpk connect --version`, which prompts you to install the plugin.
