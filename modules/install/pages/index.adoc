...
| xref:install:kubernetes-operator.adoc[Redpanda Operator (Kubernetes, beta)]
| Run pipelines as declaratively managed `Pipeline` resources on Kubernetes. Best when you already manage Redpanda with the Redpanda Operator. There is no separate operator to install.
