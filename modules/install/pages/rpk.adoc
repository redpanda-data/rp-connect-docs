...
include::streaming:get-started:partial$install-rpk-linux.adoc[]
...
include::streaming:get-started:partial$install-rpk-macos.adoc[]
...
The reverse is also true: upgrading `rpk` does not upgrade Redpanda Connect. The plugin is a separate binary that stays at its installed version, even across `rpk` upgrades, until you run `rpk connect upgrade`. The automatic install happens only on first use and installs the latest version at that moment.
